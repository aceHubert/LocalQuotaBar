...
| 2026-09-19 | 套餐判定 | ZCode 3.14.0 起套餐按 workspace 记忆、可多窗口并存（本机当日 start-plan 与 individual-coding-plan 同时活跃），切换是会话级、不落盘，已提供应用侧手动切换（右键菜单按 codex-cliproxy 文件槽位法现读 setting.json/config.json 判定可选项：start-plan 永远可选、团队叠加 provider 停用标记；槽位存在≠权益有效，切换后查询失败在面板报错）但感知不到各会话实际所用套餐，多套餐并存展示未实现；严格权益判定口径见已删除的调研结论（subscription/list KI 谓词、querySubscribeDetail 五层闸门），需要时可按 git 历史恢复网络探测 | setting.json 不落 per-workspace 选择状态；权益接口失败 ≠ 无资格，按失败自动回退会在网络抖动时面板反复横跳（见执行计划决策记录） | 参见执行计划 active/zai-dynamic-plan-selection.md：如需完整覆盖，单独立项设计多套餐并存展示（快照模型改列表 + 面板多区块）并以权益探测结果作展示输入 |
| 2026-09-22 | 额度面板 | CodeBuddy 国内版（`www.codebuddy.cn`）看板未实现，首版只做国际版（`.ai`） | 用户要求先做国际版；`.cn` 与 `.ai` 是不同登录态，双站并存形态（独立 tab vs 区域切换）待国际版落地后评估 | 见 active/quota-popup-tabs-redesign.md「推迟项」章节的方案草案；国际版上线并真实验证后单独立项实现 |
