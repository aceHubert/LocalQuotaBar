...
- [x] DeepSeek 会话过期态实测（2026-09-24 用户确认完成）。
- [x] 多 profile 显式选择 UI 与失败后候选轮换明确推迟并登记技术债，完成归档。
